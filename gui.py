...
class ems_gui:
    ...
    def refresh_dispatches(self, dispatch_listbox):
        ...
        if response.status_code == 200:
            ...
            for d in dispatches:
                display_text = (
                    f"[{d['timestamp']}] - {d.get('ambulance', 'Unknown')} dispatched at {d['incident_location']} for {d['message']} - Severity: {d['severity']}")
                dispatch_listbox.insert(tk.END, display_text)

    def calculate_route(self, origin_entry, destination_entry, route_text):
        ...
        if response.status_code == 200:
            ...
            route_text.insert(
                tk.END, f"Destination: {route_info.get('destination')}\n")
            route_text.insert(
                tk.END, f"Total Distance: {route_info.get('distance', {}).get('text')}\n")
            route_text.insert(
                tk.END, f"Time to Destination: {route_info.get('duration', {}).get('text')}\n\n")
            route_text.insert(tk.END, "Directions:\n")
            for idx, step in enumerate(route_info.get('steps', []), 1):
                route_text.insert(tk.END, f"{idx}. {step}\n")

    def suggest_hospital(self, hospital_incident_entry, hospital_result_text):
        ...
        if response.status_code == 200:
            ...
            hospital_result_text.insert(
                tk.END, f"Suggested Hospital:\nName: {hospital.get('name')}\n")
            hospital_result_text.insert(
                tk.END, f"Address: {hospital.get('address')}\n")
            hospital_result_text.insert(
                tk.END, f"Available Beds: {hospital.get('available_beds')}\n")
            hospital_result_text.insert(
                tk.END, f"Estimated Travel Time: {hospital.get('estimated_travel_time')}\n")
    ...
    def run_gui(self):
        ...
        style.configure("TButton", font=("Helvetica", 10, "bold"), padding=6,
                        background="#1b64c1", relief="raised", highlightcolor="blue")
        ...
        ttk.Label(input_frame, text="Severity:").grid(
            row=2, column=0, sticky=tk.W, pady=5)
        severity_var = tk.StringVar(value="Normal")
        severity_combo = ttk.Combobox(input_frame, textvariable=severity_var, values=[
                                      "Low", "Normal", "High"])
        ...
        dispatch_listbox = tk.Listbox(
            dispatch_tab, width=80, height=15, font=("Helvetica", 11))
        ...
        ttk.Label(route_input_frame, text="Origin:").grid(
            row=0, column=0, sticky=tk.W, pady=5)
        origin_entry = ttk.Entry(route_input_frame, width=50)
        origin_entry.grid(row=0, column=1, sticky=tk.W, pady=5)

        ttk.Label(route_input_frame, text="Destination:").grid(
            row=1, column=0, sticky=tk.W, pady=5)
        destination_entry = ttk.Entry(route_input_frame, width=50)
        destination_entry.grid(row=1, column=1, sticky=tk.W, pady=5)

        calc_button = ttk.Button(
            route_input_frame, text="Calculate Route", command=lambda: self.calculate_route(origin_entry, destination_entry, route_text))
        calc_button.grid(row=2, column=0, sticky=tk.W, pady=10, padx=20)

        route_text = tk.Text(route_tab, height=15,
                             width=80, font=("Helvetica", 11))
        ...
        ttk.Label(hosp_input_frame, text="Incident Location:").grid(
            row=0, column=0, sticky=tk.W, pady=5)
        ...
        suggest_button = ttk.Button(
            hosp_input_frame, text="Suggest Hospital", command=lambda: self.suggest_hospital(hospital_incident_entry, hospital_result_text))
        suggest_button.grid(row=1, column=0, sticky=tk.W, pady=10, padx=20)

        hospital_result_text = tk.Text(
            hospital_tab, height=10, width=80, state='disabled', font=("Helvetica", 11))
